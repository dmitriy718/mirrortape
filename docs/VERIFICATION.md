...
| F09 | Public browser-only demo plus guest form drafts; account signup preserves its draft | Chromium demo isolation, registration, verification and sign-in journeys. Demo samples are not imported into private accounts. |
...
## September 7 — repeat audit, draft reconciliation and release resilience

[Audit scope, architecture, findings and remaining requirements](AUDIT_2026-09-07.md). The twenty-feature map remains the acceptance baseline. This increment adds explicit cross-tab conflict comparison/choice, safe merging of unrelated draft edits, consistent contact topic/reference state, ordered maintenance/database shutdown, controlled startup failure cleanup and the top-level Docker deployment entry point.

Local gates: **43 backend/unit tests**, **72 Chromium cases**, ESLint and both TypeScript/build targets passed. The strengthened shutdown assertions were subsequently rerun on both viewports: the actual process begins closing while the API request is still blocked, completes the accepted request after release, and exits with code zero for SIGTERM and SIGINT. The startup-failure subprocess exits promptly without disclosing environment values. The earlier nullable-capture compile error was fixed before this successful build.

Nine release tests passed: real loopback HTTP readiness with transient failures, wrong release and malformed response; atomic mapping restoration; administrative reload failure; missing backup; and actual production cleanup control flow for verified rollback, failed reload and unhealthy previous process. Administrative commands in fault tests are isolated substitutes, not production disruptions. Two real Caddy proxy tests passed. Bash syntax/ShellCheck passed and npm audit reported zero known vulnerabilities. These checks do not replace production disaster-recovery, fault-injected rollback or sustained load testing.

The new `deploy.sh` defaults to preflight. Reruns verify consistent stable routing before changing slots; rollback restoration is atomic and retains a candidate if recovery cannot be verified. No migration or secret change is included. Exact CI, deployment and post-release public checks are recorded after execution below.

### Repeat-audit release verified on the VPS

Application commit **`9e1b06f6b25d3fcdaf3e14e208f10c55efcf91ac`** is active in the green slot at https://mirrortape.net. [Exact Linux CI run 34120296196](https://github.com/dmitriy718/mirrortape/actions/runs/34120296196) passed 43 backend/unit tests, 72 Chromium tests (3.4 minutes), all nine release checks, lint/build and the dependency audit. Earlier audit checkpoints also passed their CI runs; the final release includes the sticky-navigation/focus fix, plain-language status parsing failure recovery and visible retry errors when comparison refresh fails.

`sudo ./deploy.sh --apply` completed after the stable-process/routed-HTTPS checks and a validated pre-migration backup. **24 consecutive public HTTPS probes passed with zero failures**, observing both the previous and new release IDs. The previous blue process exited with code zero. A second identical deployment command returned “This exact release is already installed and healthy” without rebuilding or switching slots, proving the new rerun path against the live installation.

The final public smoke passed **48 desktop/mobile checks with zero console errors**, no overflow and the private route guard intact. The application/database and twelve existing CodexStore containers were healthy afterward. The new application emitted no warning/error-level log entries in the inspected five-minute window. Secret files remain mode 0600, and the production configuration fingerprint is unchanged. The local compiled runtime was restarted and its port-3100 readiness endpoint passed.

Evidence: CI artifacts, ignored `app/test-results/public-smoke.json` and screenshots; `.runtime/audit-release.log`, `.runtime/audit-final-vps-check.log`, `.runtime/release-9e1b06f6b25d3fcdaf3e14e208f10c55efcf91ac.json`. No production signup, support submission, payment or brokerage action was created for smoke verification. The live status check still reports SMTP, billing, Alpaca and Google/Apple/Facebook unavailable. Provider acceptance, broader trading implementation and commercial/operational launch gates remain incomplete; this is not a “SYSTEM PRODUCTION-READY” certification.
