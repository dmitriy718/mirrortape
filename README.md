...
See [social provider setup](docs/SOCIAL_AUTH_SETUP.md) and [conversion research](docs/CONVERSION_RESEARCH.md). Social sign-in requires provider credentials; email delivery requires SMTP.
...
- Separate interactive demo and authenticated private dashboard; watchlist, five-second server-enforced undo, persisted onboarding/forms, conflict detection, safe navigation, support intake and dark/light/system themes.
- PostgreSQL-backed authenticated sessions, email/password signup/login, Google/Apple/Facebook integration code with explicit identity linking, email verification/recovery, encrypted outbox/brokerage tokens, tenant isolation, shared rate limits, origin/CSRF enforcement and restrictive script CSP.
...
See [verification and remaining work](docs/VERIFICATION.md), the [full roadmap](PRODUCTION_READINESS_PLAN.md), [delivery contract](EXECUTION_CONTRACT.md), and [VPS Docker runbook](deploy/VPS_DOCKER.md).
